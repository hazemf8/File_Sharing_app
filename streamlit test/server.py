...
def handle_client(conn, addr):
    ...
    while True:
        ...
        # ---------- DELETE command ----------
            ...
            parts = command.split()
            if len(parts) != 2:
                conn.send(b"ERROR: Invalid DELETE format. Use: DELETE <filename>")
                continue
            ...
            if not os.path.isfile(filepath):
                ...
                try:
                    os.remove(filepath)
                    conn.send(f"SUCCESS: Deleted {filename}".encode())
                    print(f"[DELETE] {filename} removed by {addr}")
                except Exception as e:
                    conn.send(f"ERROR: Could not delete file. {str(e)}".encode())
        ...
        # ---------- QUIT command ----------
            ...
            conn.send(b"ERROR: Unknown command. Use LIST, UPLOAD, DOWNLOAD, DELETE, QUIT.")
